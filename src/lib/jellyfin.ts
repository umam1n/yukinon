...
export async function getGenres(searchTerm?: string) {
  if (!config || !api) throw new Error('Jellyfin config not set')
  const safeUrl = config.url.endsWith('/') ? config.url.slice(0, -1) : config.url;
  ...
  const res = await fetch(`${safeUrl}/MusicGenres?${params.toString()}`)
  if (!res.ok) throw new Error('Failed to fetch genres')
  return await res.json()
}

export async function getSongsByGenre(genreName: string) {
  if (!config || !api) throw new Error('Jellyfin config not set')
  const safeUrl = config.url.endsWith('/') ? config.url.slice(0, -1) : config.url;
  ...
  const res = await fetch(`${safeUrl}/Users/${config.userId}/Items?${params.toString()}`)
  if (!res.ok) throw new Error('Failed to fetch songs by genre')
  return await res.json()
}
